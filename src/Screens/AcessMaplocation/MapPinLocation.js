...
const MapPinLocation = ({navigation}) => {
  ...
  const {
    ...
    pickerPoint2,
    setPickerPoint2,
    setPickerPoint1Coordinates,
    setPickerPoint2Coordinates,
  ...
  const onPressAddress = (data, details = null, isPicker1 = true) => {
    ...
    if (isPicker1) {
      setCoord(location);
      // setPickerPoint1(data.description);
      // setPickerPoint1Coordinates(location);
    } else {
      setDestination(location);
      // setPickerPoint2(data.description);
      // setPickerPoint2Coordinates(location);
    }
  ...
  const fetchAddressFromCoordinates = async (latitude, longitude) => {
    try {
      ...
      if (response.data.results.length > 0) {
        ...
        setPickerPoint1(address);
        setPickerPoint1Coordinates({latitude, longitude});
      }
    } catch (error) {
      console.error(error);
    }
